...
from oms_sensemaking.clients.instances import get_db_session
...
def create_relationship(
    db: Annotated[Session, Depends(get_db_session)], relationship: Relationship
) -> CreateObjectResponse:
    """Create a relationship in the graph."""
    return CreateObjectResponse(success=True)


@router.delete("/relationship/{relationship_id}", response_model=DeleteObjectResponse, response_model_exclude_none=True)
def delete_relationship(db: Annotated[Session, Depends(get_db_session)], relationship_id: str) -> DeleteObjectResponse:
