...
class MilSymbol2525B(MilSymbol):
    ...
    MIL_SYM_2525B_DIMENSION_IDX = 2
    MIL_SYM_2525B_STATUS_IDX = 3
        ...
        if affiliation_attr:
            ...
            for code, standard_identity_list in self.settings["MIL_SYMBOL_2525B"]["STANDARD_IDENTITY_LISTS"].items():
                if node_standard_identity.lower() in standard_identity_list:
                    self.update_code(self.MIL_SYM_2525B_STD_IDENTITY_IDX, code)
    ...
    def enrich_dimension(self, oms_node: NodeNode, ancestor_iris: List[str]) -> None:
        ...
        def update_dimension(current_iri: str) -> bool:
            ...
            for code, dimension_iris in self.settings["MIL_SYMBOL_2525B"]["DIMENSION_IRIS"].items():
                dimension_iris = [dimension_iri.lower() for dimension_iri in dimension_iris]
                if current_iri in dimension_iris:
                    self.update_code(self.MIL_SYM_2525B_DIMENSION_IDX, code)
        ...
        dimension_code = self.code[self.MIL_SYM_2525B_DIMENSION_IDX]
    ...
    def enrich_status(self, status_attr: Optional[AttributeAttribute]) -> None:
        ...
        if status_attr:
            ...
            for code, status_list in self.settings["MIL_SYMBOL_2525B"]["STATUS_LISTS"].items():
                if status.lower() in status_list:
                    self.update_code(self.MIL_SYM_2525B_STATUS_IDX, code)
