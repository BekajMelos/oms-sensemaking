...
to each point, based on the ATOMS observation's confidence attribute.
...
## Usage
The Geospatial Controller is responsible for getting new ATOMS observations using the audit log, creating a Point model
...
algorithms and are never stored in ATOMS. All Points are given an initial weight by the controller based entirely on the
...
The resulting tracks are not published to ATOMS but form the inputs for the various SenseMakers to analyze. In the near
term, we're going to include the ATOMS observation IDs of all the inputs to track weaver with its output tracks so that
