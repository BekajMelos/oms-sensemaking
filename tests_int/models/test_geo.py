...
TRACK_ID: UUID = UUID("5f2fa1af-a602-4411-8b76-3a9b1f70ae1d")
TRACK_ID_FFX: UUID = UUID("f96437e0-bb67-4f59-8b0c-61a01cc5b870")

DATA: list = [  # Latitude, Longitude, Altitude (m), Description, Node ID, Obs ID, track_id
    [34.052235, -118.243683, 100, "Central Los Angeles, downtown area", NODE_ID, uuid4(), TRACK_ID],
    [34.052500, -118.245000, 150, "Near Los Angeles State Historic Park", NODE_ID, uuid4(), TRACK_ID],
    [34.053000, -118.247000, 120, "Close to Chinatown", NODE_ID, uuid4(), TRACK_ID],
    [34.054000, -118.249000, 180, "Elysian Park entrance", NODE_ID, uuid4(), TRACK_ID],
    [34.055000, -118.251000, 200, "Elysian Park trailhead", NODE_ID, uuid4(), TRACK_ID],
    [34.056000, -118.253000, 250, "Midpoint of trail, higher elevation", NODE_ID, uuid4(), TRACK_ID],
    [34.057000, -118.255000, 230, "Near Dodger Stadium", NODE_ID, uuid4(), TRACK_ID],
    [34.058000, -118.257000, 190, "Downhill from Dodger Stadium", NODE_ID, uuid4(), TRACK_ID],
    [34.059000, -118.259000, 160, "Park exit", NODE_ID, uuid4(), TRACK_ID],
    [34.060000, -118.261000, 140, "Residential area", NODE_ID, uuid4(), TRACK_ID],
    [34.061000, -118.263000, 110, "End of the route, nearby school", NODE_ID, uuid4(), TRACK_ID],
    [34.062000, -118.265000, 90, "Final point, local marketplace", NODE_ID, uuid4(), TRACK_ID],
    [38.846224, -77.306373, None, "Fairfax, VA", NODE_ID_FFX, ATTR_ID_FFX, TRACK_ID_FFX],
]
...
def test_get_points_track(tester_db: Session):
    points = get_track_points(tester_db, TRACK_ID)
...
def test_get_track(tester_db: Session):
    track: Track = get_track(tester_db, TRACK_ID)

    assert track
    assert len(track.points) == 12
    assert track.points[0].node_id == NODE_ID
    assert track.points[0].observation_id == DATA[0][5]
...
def test_get_track_uuid_str(tester_db: Session):
    track: Track = get_track(tester_db, "5f2fa1af-a602-4411-8b76-3a9b1f70ae1d")
